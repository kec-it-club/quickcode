...
QuizRenderer = function (topElementSelector) {
	...
	if (!topElem.length) {
		throw (
			"QuizRenderer: could not find topElement with selector " +
			topElementSelector
		);
	}
	...
	this.renderQuestion = function (question, clickcallback) {
		...
		topElem
			.empty()
			.append($("<div>").attr("id", "questiontext").text(question.text));
		...
		if (question.image) {
			...
			topElem.append(
				canvas.attr("id", "questionimage").css("display", "block")
			);
			img = new Image();
			img.onload = function () {
		...
		if (
			(!question.type || question.type == 0 || question.type == 2) &&
			question.options
		) {
...
QuizUtil = {
	...
	permaId: function () {
		var id = localStorage.getItem("QuickCodeId");
		if (!id) {
			id = QuizUtil.randomString();
			localStorage.setItem("QuickCodeId", id);
		}
		return id;
	},
